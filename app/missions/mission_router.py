...
from .mission_action import _create_task, _get_all_task, _update_task, _delete_task
...
async def create_task(body: TaskCreate,
    ...
    created_task: Task = await _create_task(session=session,
                            owner_id=current_user.id, **task)
    return created_task
...
async def get_all_task(curent_user: TokenData = Depends(get_current_user),
    ...
    tasks: list[Task] = await _get_all_task(session=session, owner_id=curent_user.id)
...
async def update_task(body: TaskUpdate,
    ...
    updated_task: Task = await _update_task(session=session,
                                            owner_id=current_user.id, **task)
    return updated_task
...
async def delete_task(task_id: int, 
    ...
    if task_id is None:
        raise HTTPException(403,
                            detail=f"the request field is empty")
    deleted_task_id = await _delete_task(session, task_id, 
                                         current_user.id)
